...
type ViewMode = "sheet" | "list" | "classSelect";

/* ── Derived HP from ODO modifier ── */
...
export default function CharactersPage() {
  ...
  const [classSelectMode, setClassSelectMode] = useState<"create" | "levelup">("create");
  ...
  /* ── CLASS SELECT (výber povolania — konzistentný s layoutom) ── */
  if (view === "classSelect") {
    return (
      <div className="fixed inset-0 z-50 overflow-y-auto" style={{ background: "var(--bg-deep)" }}>
        <ClassSelector
          mode={classSelectMode}
          onSelect={(ch) => {
            setClassPreselect(ch);
            setView("sheet");
          }}
          onCancel={() => { setView("list"); setClassSelectMode("create"); }}
        />
      </div>
    );
  }
    ...
    <div className="max-w-3xl">
        ...
        <div className="flex gap-2">
          {!isRoster && (
            <button onClick={() => setRosterModalOpen(true)}
              className="flex items-center gap-2 font-medium text-sm px-3 py-2 rounded-lg transition-all dh-input"
            >
              📋 Pridať z rosteru
            </button>
          )}
          <button onClick={() => { setClassSelectMode("create"); setView("classSelect"); }}
            className="flex items-center gap-2 dh-btn-primary font-semibold text-sm px-3 py-2 rounded-lg transition-all"
          >+ Nová postava</button>
        </div>
      </div>
      ...
      {rosterModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{ background: "rgba(0,0,0,0.7)" }} onClick={() => setRosterModalOpen(false)}>
          <div className="rounded-xl p-6 w-full max-w-lg max-h-[80vh] overflow-hidden flex flex-col" style={{ background: "var(--bg-card)", border: "1px solid var(--border-default)" }} onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-semibold mb-2" style={{ color: "var(--text-primary)" }}>Pridať postavu z rosteru</h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-2">
              {rosterChars.length === 0 ? (
                <p className="text-sm py-4 text-center" style={{ color: "var(--text-muted)" }}>Roster je prázdny. Vytvor postavy v sekcii Postavy.</p>
              ) : (
                rosterChars.map((rc) => (
                  <button
                    key={rc.id}
      ...
      {characters.length === 0 && (
        <div className="text-center py-16 rounded-xl" style={{ border: "1px dashed var(--border-default)" }}>
          ...
          <p className="text-sm mb-4" style={{ color: "var(--text-muted)" }}>Žiadne postavy v tejto kampani.</p>
          <button onClick={() => { setClassSelectMode("create"); setView("classSelect"); }}
            className="text-sm underline underline-offset-2"
            style={{ color: "var(--accent-gold)" }}
          >Vytvoriť prvú postavu →</button>
        </div>
      )}
    </div>
  );
